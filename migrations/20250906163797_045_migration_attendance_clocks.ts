...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(`
    CREATE TRIGGER update_attendance_clocks_updated_at
    BEFORE UPDATE ON attendance_clocks
    FOR EACH ROW
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw('DROP TRIGGER IF EXISTS update_attendance_clocks_updated_at ON attendance_clocks;');
  await knex.raw('DROP FUNCTION IF EXISTS update_updated_at_column;');
  await knex.schema.dropTable('attendance_clocks');
}
