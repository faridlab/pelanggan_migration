...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('product_variant_units', (table) => {
  ...
  await knex.raw(`
    CREATE TRIGGER update_product_variant_units_updated_at
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw('DROP TRIGGER IF EXISTS update_product_variant_units_updated_at ON product_variant_units;');
  await knex.raw('DROP FUNCTION IF EXISTS update_updated_at_column;');
  await knex.schema.dropTable('product_variant_units');
}
