...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('product_identifications', (table) => {
  ...
  await knex.raw(`
    CREATE TRIGGER update_product_identifications_updated_at
    BEFORE UPDATE ON product_identifications
    FOR EACH ROW
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw('DROP TRIGGER IF EXISTS update_product_identifications_updated_at ON product_identifications;');
  await knex.raw('DROP FUNCTION IF EXISTS update_updated_at_column;');
  await knex.schema.dropTable('product_identifications');
}
