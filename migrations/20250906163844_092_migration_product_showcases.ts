...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('product_showcases', (table) => {
  ...
  await knex.raw(`
    ...
    BEFORE UPDATE ON product_showcases
    FOR EACH ROW
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw('DROP TRIGGER IF EXISTS update_product_showcases_updated_at ON product_showcases;');
  await knex.raw('DROP FUNCTION IF EXISTS update_updated_at_column;');
  await knex.schema.dropTable('product_showcases');
}
