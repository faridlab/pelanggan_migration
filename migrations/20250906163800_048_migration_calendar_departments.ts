...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(`
    CREATE TRIGGER update_calendar_departments_updated_at
    BEFORE UPDATE ON calendar_departments
    FOR EACH ROW
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw('DROP TRIGGER IF EXISTS update_calendar_departments_updated_at ON calendar_departments;');
  await knex.raw('DROP FUNCTION IF EXISTS update_updated_at_column;');
  await knex.schema.dropTable('calendar_departments');
}
