...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(`
    CREATE TRIGGER update_contact_socials_updated_at
    BEFORE UPDATE ON contact_socials
    FOR EACH ROW
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw('DROP TRIGGER IF EXISTS update_contact_socials_updated_at ON contact_socials;');
  await knex.raw('DROP FUNCTION IF EXISTS update_updated_at_column;');
  await knex.schema.dropTable('contact_socials');
}
