...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(`
    CREATE TRIGGER update_employee_identities_updated_at
    BEFORE UPDATE ON employee_identities
    FOR EACH ROW
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw('DROP TRIGGER IF EXISTS update_employee_identities_updated_at ON employee_identities;');
  await knex.raw('DROP FUNCTION IF EXISTS update_updated_at_column;');
  await knex.schema.dropTable('employee_identities');
}
